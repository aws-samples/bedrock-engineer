...
| `cameraCapture`   | コンピューターのカメラを起動して画像をキャプチャし、ファイルとして保存します。内蔵カメラと外付けWebカメラの両方をサポートします。オプションでビジョンモデル（Claude/Nova）を使用してAI分析を行い、テキスト内容の抽出、オブジェクトの認識、人物の識別、QRコード/バーコードの読み取り、詳細な視覚的説明を提供します。カメラへのアクセスにはブラウザ/システム権限ダイアログを通じて明示的なユーザー許可が必要です。                                                                     |
