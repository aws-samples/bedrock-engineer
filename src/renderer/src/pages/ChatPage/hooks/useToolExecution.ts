...
export function useToolExecution({
   ...
   * ツールを再帰的に実行
   */
  const recursivelyExecTool = useCallback(
    async (
      contentBlocks: ContentBlock[],
      currentMessages: Message[],
      callbacks: ToolExecutionCallbacks
    ) => {
      const { setExecutingTool, setMessages, streamChat } = callbacks
      ...
      if (stopReason === 'tool_use') {
        ...
        if (lastMessage) {
          await recursivelyExecTool(lastMessage, currentMessages, callbacks)
          return
        }
      }
    },
    [guardrailSettings, persistMessage, enabledTools, modelId, systemPrompt, t]
  )
