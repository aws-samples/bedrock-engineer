...
| `cameraCapture`   | Activates the computer's camera to capture an image and save it as a file. Supports both built-in and external webcams. Optionally analyzes the captured image with AI using vision models (Claude/Nova) to extract text content, recognize objects, identify people, read QR codes/barcodes, and provide detailed visual descriptions. Camera access requires explicit user permission through browser/system permissions dialog.                                                                                               |
