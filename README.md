...
| `screenCapture`   | Captures the current screen and saves it as a PNG image file. Optionally can use an AI vision model (Claude/Nova) to analyze the captured image, extracting text contents, identifying UI elements, and providing detailed visual descriptions for debugging or documentation purposes. Requires platform-specific permissions (macOS: Screen Recording permission in System Preferences).                                                                                                                                                       |
| `cameraCapture`   | Activates the computer's camera to capture images and save them as files. Supports both built-in cameras and external webcams. Can optionally perform AI analysis using vision models (Claude/Nova) to extract text content, recognize objects, identify people, read QR codes/barcodes, and provide detailed visual descriptions. Requires explicit user permission through browser/system permission dialogs to access the camera.                                                                                   |
